...
{
    DEBUG_ENTRY("const mcl_uint8_t *data = <%p>, mcl_size_t data_size = <%u>, const mcl_uint8_t *key = <%p>, mcl_size_t key_size = <%u>, mcl_uint8_t **hash = <%p>, mcl_size_t *hash_size = <%p>",
	...
	ASSERT_CODE_MESSAGE(key_size > 0, MCL_INVALID_PARAMETER, "Size of key for HMAC must be greater than 0!");
    ...
    if (key_size > HMAC_MAXIMUM_KEY_SIZE)
    {
        ...
        ASSERT_CODE_MESSAGE(MCL_OK == code, code, "Given HMAC key size = <%u> was greater than allowed = <%u>, but new key couldn't be constructed from its SHA256 hash!",
                            key_size, HMAC_MAXIMUM_KEY_SIZE);
        ASSERT_STATEMENT_CODE_MESSAGE(HMAC_SHA256_SIZE == key_local_size, MCL_FREE(key_resized), MCL_SHA256_CALCULATION_FAIL,
