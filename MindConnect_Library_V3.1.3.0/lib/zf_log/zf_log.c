...
void zf_log_out_stderr_callback(const zf_log_message *const msg, void *arg)
{
	...
	WriteFile(GetStdHandle(STD_ERROR_HANDLE), msg->buf,
			  (DWORD)(msg->p - msg->buf + eol_len), &numberOfBytesWritten, 0);
#else
...
static void time_callback(struct tm *const tm, unsigned *const msec)
{
	...
	*msec = st.wMilliseconds;
    ...
    #endif
	//increase month in order to start from 1 like in windows
    tm->tm_mon += 1;
...
static INLINE char *put_string_r(const char *s, char *p)
{
    size_t n;
	for (n = 0; s[n] != '\0'; n++);
	char *const e = p - n;
	char *const c = (char *)memccpy(e, s, '\0', n);
    return e;
}
...
static void put_ctx(zf_log_message *const msg)
{
	...
	n = snprintf(msg->p, nprintf_size(msg),
				 "%04u-%02u-%02u %02u:%02u:%02u,%03u | %05i | %05i | %s | ",
				 (unsigned)(tm.tm_year),
	...
	char *p = e;
	char *l = lvl_char(msg->lvl);
    const char* separator = " | \0";
    p = put_string_r(separator, p);
	p = put_string_r(l, p);
    p = put_string_r(separator, p);
	p = put_int_r(tid, 5, '0', p);
	p = put_string_r(separator, p);
	p = put_int_r(pid, 5, '0', p);
	p = put_string_r(separator, p);
    ...
    *--p = '-';
    p = put_uint_r((unsigned)tm.tm_mon, 2, '0', p);
    *--p = '-';
    p = put_uint_r((unsigned)tm.tm_year, 4, '0', p);
...
{
    const char *file = filename(src->file);
    char *file_copy = (char*)malloc(strlen(file) + 1);
    if(NULL == file_copy) {
        return;
    }
    strcpy(file_copy, file);
    const char *basename = strtok(file_copy, ".");
#if ZF_LOG_OPTIMIZE_SIZE
	int n;
	n = snprintf(msg->p, nprintf_size(msg), "%s.%s | %u | ",
				 funcname(src->func), basename, src->line);
	put_nprintf(msg, n);
#else
	msg->p = put_string(basename, msg->p, msg->e);
	if (msg->p < msg->e) *msg->p++ = '.';
