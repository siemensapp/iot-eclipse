 ...
 */
	...
	#define ZF_LOG_WRITE(lvl, tag, ...) \
			do { \
				if (ZF_LOG_ON(lvl)) \
					_zf_log_write_d(_ZF_LOG_FUNCTION, __FILE__, (unsigned int)__LINE__, \
							lvl, tag, __VA_ARGS__); \
			} _ZF_LOG_ONCE
	#define ZF_LOG_WRITE_MEM(lvl, tag, d, d_sz, ...) \
			do { \
				if (ZF_LOG_ON(lvl)) \
					_zf_log_write_mem_d(_ZF_LOG_FUNCTION, __FILE__, (unsigned int)__LINE__, \
							lvl, tag, d, d_sz, __VA_ARGS__); \
			} _ZF_LOG_ONCE
	#define ZF_LOG_WRITE_AUX(log, lvl, tag, ...) \
			do { \
				if (ZF_LOG_ON(lvl)) \
					_zf_log_write_aux_d(_ZF_LOG_FUNCTION, __FILE__, (unsigned int)__LINE__, \
							log, lvl, tag, __VA_ARGS__); \
			} _ZF_LOG_ONCE
	#define ZF_LOG_WRITE_MEM_AUX(log, lvl, tag, d, d_sz, ...) \
			do { \
				if (ZF_LOG_ON(lvl)) \
					_zf_log_write_mem_aux_d(_ZF_LOG_FUNCTION, __FILE__, (unsigned int)__LINE__, \
