...
*************************************************************************
*
* @file     test_rotate_key.c
* @date     Oct 27, 2016
* @brief    An integration test for testing key rotation functionality of the MCL.
*
************************************************************************/
 ...
 * WHEN  : Agent tries to rotate its authentication key.
 ...
 * WHEN  : Agent tries to rotate its authentication key.
 * THEN  : HTTP Response 200 is returned from server and the key is rotated.
 */
void test_rotate_key_02(void)
{
    ...
    if (MCL_OK == code)
    {
        ...
        TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Agent could not onboard before key rotation.");
    }
    ...
    TEST_ASSERT_EQUAL_MESSAGE(MCL_OK, code, "Server did not respond correctly to regular key rotation.");
}
