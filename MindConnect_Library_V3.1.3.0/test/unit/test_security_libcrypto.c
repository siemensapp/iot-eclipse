 ...
 *************************************************************************
 *
 * @file     test_security_libcrypto.c
 * @date     Aug 2, 2016
 * @brief    Test module for testing security handler implemented by use of OpenSSL.
 *
 ************************************************************************/

#include "mcl/mcl_config_setup.h"

#ifdef MCL_HAVE_OPENSSL
...
/**
 * GIVEN : Test data to calculate SHA256 for.
 * WHEN  : SHA256 calculation called.
...
void test_sha256_001()
{
    ...
    string_initialize_static("The quick brown fox jumps over the lazy dog", 0, &data);

    E_MCL_ERROR_CODE code = security_hash_sha256((mcl_uint8_t *)data->buffer, data->length, &hash, &hash_size);
    ...
    TEST_ASSERT_MESSAGE(MCL_OK == code, "Expected return code not received");
    TEST_ASSERT_EQUAL_STRING_MESSAGE("D7A8FBB307D7809469CA9ABCB0082E4F8D5651E46D3CDB762D02D0BF37C9E592", hex_hash->buffer, "Expected SHA256 not achieved!");
...
/**
 * GIVEN : empty test data to calculate SHA256 for.
 * WHEN  : SHA256 calculation called.
 * THEN  : Expected SHA256 with correct size must be returned.
 */
void test_hash_sha256_002(void)
{
    ...
    mcl_uint8_t *hash = MCL_NULL;
    mcl_size_t hash_size = 0;
    string_initialize_static("", 0, &data);
    ...
    TEST_ASSERT_MESSAGE(MCL_OK == code, "Expected return code not received");
    TEST_ASSERT_EQUAL_STRING_MESSAGE("E3B0C44298FC1C149AFBF4C8996FB92427AE41E4649B934CA495991B7852B855", hex_hash->buffer, "Expected SHA256 not achieved!");
...
/**
 * GIVEN : Nothing.
 * WHEN  : RSA key pair is generated.
...
void test_generate_rsa_key_001(void)
{
    ...
    E_MCL_ERROR_CODE code = security_generate_rsa_key(&public_key, &private_key);
 ...
 * WHEN  : Data is signed with the given RSA key.
...
void test_rsa_sign_001(void)
{
    ...
    E_MCL_ERROR_CODE code = security_rsa_sign(private_key, data, strlen(data), &signature, &signature_length);
 ...
 * WHEN  : Modulus and public exponent are extracted.
...
void test_rsa_get_modulus_and_exponent_001(void)
{
    ...
    E_MCL_ERROR_CODE code = security_rsa_get_modulus_and_exponent(public_key, &modulus, &exponent);
